...
function m.bottom(msg, id, color, sound)
    if id == nil then
        CustomGameEventManager:Send_ServerToAllClients(
            PlayerResource:GetPlayer(id), 'msg_bottom',
            {Message = tostring(msg), Color = color, Sound = sound})
        return
    end
    CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(id),
                                               'msg_bottom', {
